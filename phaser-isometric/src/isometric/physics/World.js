...
import {ISO_SPRITE} from '../IsoSprite';
...
export default class World {
   ...
   * @param {object|array|GameObjects.Group} object - The game object to create the physics body on.
   * Can also be an array or Group of objects, a body will be created on every child that has a `body` property.
  ...
  separate(body1, body2, processCallback, callbackContext, overlapOnly) {
    if (!body1.enable || !body2.enable || !World.intersects(body1, body2)) {
      return false;
    }
   ...
   * @param {Body} bodyA - The Body object to separate.
   * @param {Body} bodyB - The Body object to separate.
   * @param {boolean} overlapOnly - If true the bodies will only have their overlap data set, no separation or exchange of velocity will take place.
   * @returns {boolean} Returns true if the bodies were separated, otherwise false.
   */
  separateX(bodyA, bodyB, overlapOnly) {
    ...
    this._maxOverlap = bodyA.deltaAbsX() + bodyB.deltaAbsX() + this.OVERLAP_BIAS;

    if (bodyA.deltaX() === 0 && bodyB.deltaX() === 0) {
      //  They overlap but neither of them are moving
      bodyA.embedded = true;
      bodyB.embedded = true;
    } else if (bodyA.deltaX() > bodyB.deltaX()) {
      //  Body1 is moving forward and/or Body2 is moving back
      this._overlap = bodyA.frontX - bodyB.x;

      if ((this._overlap > this._maxOverlap) || bodyA.checkCollision.frontX === false || bodyB.checkCollision.backX === false) {
        this._overlap = 0;
      } else {
        bodyA.touching.none = false;
        bodyA.touching.frontX = true;
        bodyB.touching.none = false;
        bodyB.touching.backX = true;
      }
    } else if (bodyA.deltaX() < bodyB.deltaX()) {
      //  Body1 is moving back and/or Body2 is moving forward
      this._overlap = bodyA.x - bodyB.widthX - bodyB.x;

      if ((-this._overlap > this._maxOverlap) || bodyA.checkCollision.backX === false || bodyB.checkCollision.frontX === false) {
        this._overlap = 0;
      } else {
        bodyA.touching.none = false;
        bodyA.touching.backX = true;
        bodyB.touching.none = false;
        bodyB.touching.frontX = true;
      }
    }

    //  Then adjust their positions and velocities accordingly (if there was any overlap)
    if (this._overlap !== 0) {
      bodyA.overlapX = this._overlap;
      bodyB.overlapX = this._overlap;

      if (overlapOnly || bodyA.customSeparateX || bodyB.customSeparateX) {
        return true;
      }

      this._velocity1 = bodyA.velocity.x;
      this._velocity2 = bodyB.velocity.x;

      if (!bodyA.immovable && !bodyB.immovable) {
        this._overlap *= 0.5;

        bodyA.x = bodyA.x - this._overlap;
        bodyB.x += this._overlap;

        this._newVelocity1 = Math.sqrt((this._velocity2 * this._velocity2 * bodyB.mass) / bodyA.mass) * ((this._velocity2 > 0) ? 1 : -1);
        this._newVelocity2 = Math.sqrt((this._velocity1 * this._velocity1 * bodyA.mass) / bodyB.mass) * ((this._velocity1 > 0) ? 1 : -1);
        this._average = (this._newVelocity1 + this._newVelocity2) * 0.5;
        this._newVelocity1 -= this._average;
        this._newVelocity2 -= this._average;

        bodyA.velocity.x = this._average + this._newVelocity1 * bodyA.bounce.x;
        bodyB.velocity.x = this._average + this._newVelocity2 * bodyB.bounce.x;
      } else if (!bodyA.immovable) {
        bodyA.x = bodyA.x - this._overlap;
        bodyA.velocity.x = this._velocity2 - this._velocity1 * bodyA.bounce.x;
      } else if (!bodyB.immovable) {
        bodyB.x += this._overlap;
        bodyB.velocity.x = this._velocity1 - this._velocity2 * bodyB.bounce.x;
      }
  ...
  overlap(object1, object2, overlapCallback = null, processCallback = null, callbackContext) {
    ...
    if (Array.isArray(object2)) {
      for (let i = 0, len = object2.length; i < len; i++) {
   ...
   * @param {object} [callbackContext] - The context in which to run the callbacks.
   * @returns {boolean} True if a collision Phaser.GameObjects.Group otherwise false.
   */
  ...
  collideHandler(object1, object2, collideCallback, processCallback, callbackContext, overlapOnly) {
    ...
    if (!object2 && object1.type === Phaser.GameObjects.GROUP) {
    ...
    //  ISOSPRITES
    if (object1.type === ISO_SPRITE) {
      if (object2.type === ISO_SPRITE) {
    ...
    //  GROUPS
    else if (object1.type === Phaser.GameObjects.GROUP) {
      if (object2.type === ISO_SPRITE) {
  ...
  collideGroupVsGroup(group1, group2, collideCallback, processCallback, callbackContext, overlapOnly) {
    ...
    for (let i = 0, len = group1.children.size; i < len; i++) {
  ...
  update(time, delta) {
    this.bodies.entries.filter(body => body.enable).forEach(body => body.update(time, delta));
  }

  postUpdate() {
    this.bodies.entries.filter(body => body.enable).forEach(body => body.postUpdate());
  }
}
