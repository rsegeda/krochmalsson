...
export default class Body {
  ...
  constructor(sprite) {
     ...
     * @name Body#halfWidthX
     * @type {number}
     */
    this.halfWidthX = Math.abs(this.widthX * 0.5);

    /**
     * @name Body#halfWidthY
     * @property {number} Body#halfWidthX - The calculated X width / 2 of the physics body.
     */
    this.halfWidthY = Math.abs(this.widthY * 0.5);

    /**
     * @name Body#halfHeight
     * @property {number} Body#halfHeight - The calculated height / 2 of the physics body.
     */
    this.halfHeight = Math.abs(this.height * 0.5);

    /**
     * @name Body#center
     * @property {Point3} Body#center - The center coordinate of the physics body.
     */
    this.center = new Point3(sprite.isoX + this.halfWidthX, sprite.isoY + this.halfWidthY, sprite.isoZ + this.halfHeight);

    /**
     * @name Body#velocity
     * @property {Point3} Body#velocity - The velocity in pixels per second sq. of the Body.
     */
    this.velocity = new Point3();

    /**
     * @name Body#newVelocity
     * @property {Point3} Body#newVelocity - New velocity.
     * @readonly
     */
    ...
    /**
     * @name Body#deltaMax
     * @property {Point3} Body#deltaMax - The Sprite position is updated based on the delta x/y values.
     * You can set a cap on those (both +-) using deltaMax.
     */
    this.deltaMax = new Point3();

    /**
     * @name Body#acceleration
     * @property {Point3} Body#acceleration - The velocity in pixels per second sq. of the Body.
     */
    this.acceleration = new Point3();

    /**
     * @name Body#drag
     * @property {Point3} Body#drag - The drag applied to the motion of the Body.
     */
    this.drag = new Point3();

    /**
     * @name Body#allowGravity
     * @property {boolean} Body#allowGravity - Allow this Body to be influenced by gravity? Either world or local.
     * @default
     */
    ...
    /**
     * @name Body#gravity
     * @property {Point3} Body#gravity - A local gravity applied to this Body.
     * If non-zero this over rides any world gravity, unless Body.allowGravity is set to false.
     */
    this.gravity = new Point3();

    /**
     * @name Body#bounce
     * @property {Point3} Body#bounce - The elasticitiy of the Body when colliding. bounce.x/y/z = 1 means full rebound,
     * bounce.x/y/z = 0.5 means 50% rebound velocity.
     */
    this.bounce = new Point3();

    /**
     * @name Body#maxVelocity
     * @property {Point3} Body#maxVelocity - The maximum velocity in pixels per second sq. that the Body can reach.
     * @default
     */
     ...
     * @property {number} Body#angularVelocity - The angular velocity in pixels per second sq. of the Body.
     * @default
     */
    this.angularVelocity = 0;

    /**
     * @name Body#angularAcceleration
     * @property {number} Body#angularAcceleration - The angular acceleration in pixels per second sq. of the Body.
     * @default
     */
    this.angularAcceleration = 0;

    /**
     * @name Body#angularDrag
     * @property {number} Body#angularDrag - The angular drag applied to the rotation of the Body.
     * @default
     */
    this.angularDrag = 0;

    /**
     * @name Body#maxAngular
     * @property {number} Body#maxAngular - The maximum angular velocity in pixels per second sq. that the Body can reach.
     * @default
     */
    ...
    /**
     * @name Body#mass
     * @property {number} Body#mass - The mass of the Body.
     * @default
     */
    this.mass = 1;
    ...
    /**
     * @name Body#angle
     * @property {number} Body#angle - The angle of the Body in radians as calculated by its velocity, rather than its visual angle.
     * @readonly
     */
    this.angle = 0;
    ...
    /**
     * @name Body#speed
     * @property {number} Body#speed - The speed of the Body as calculated by its velocity.
     * @readonly
     */
    this.speed = 0;
    ...
    /**
     * @name Body#facing
     * @property {number} Body#facing - A const reference to the direction the Body is traveling or facing.
     * @default
     */
    ...
    /**
     * @name Body#immovable
     * @property {boolean} Body#immovable - An immovable Body will not receive any impacts from other bodies.
     * @default
     */
     ...
     * @name Body#moves
     * @property {boolean} Body#moves - Set to true to allow the Physics system to move this Body, other false to move it manually.
     * @default
     */
    this.moves = true;
     ...
     * @name Body#customSeparateX
     * @property {boolean} Body#customSeparateX - Use a custom separation system or the built-in one?
     * @default
     */
     ...
     * Used in combination with your own collision processHandler you can create whatever type of collision response you need.
     * @name Body#customSeparateY
     * @property {boolean} Body#customSeparateY - Use a custom separation system or the built-in one?
     * @default
     */
     ...
     * Used in combination with your own collision processHandler you can create whatever type of collision response you need.
     * @name Body#customSeparateZ
     * @property {boolean} Body#customSeparateZ - Use a custom separation system or the built-in one?
     * @default
     */
     ...
     * @name Body#overlapX
     * @property {number} Body#overlapX - The amount of horizontal overlap during the collision.
     */
    this.overlapX = 0;
     ...
     * When this body collides with another, the amount of overlap is stored here.
     * @name Body#overlapY
     * @property {number} Body#overlapY - The amount of vertical overlap during the collision.
     */
    this.overlapY = 0;
     ...
     * @name Body#embedded
     * @property {boolean} Body#embedded - Body embed value.
     */
    this.embedded = false;
     ...
     * @property {boolean} Body#collideWorldBounds - Should the Body collide with the World bounds?
     */
     ...
     * @name Body#checkCollision
     * @property {object} Body#checkCollision - An object containing allowed collision.
     */
    this.checkCollision = {
      none: false,
      any: true,
     ...
     * @name Body#touching
     * @property {object} Body#touching - An object containing touching results.
     */
    this.touching = {
      none: true,
      up: false,
     ...
     * @property {object} Body#wasTouching - An object containing previous touching results.
     */
    this.wasTouching = {
      none: true,
      up: false,
     ...
     * @property {object} Body#blocked - An object containing on which faces this Body is blocked from moving, if any.
     */
    this.blocked = {
      up: false,
      down: false,
    ...
    /**
     * @property {number} Body#phase - Is this Body in a update (1) or postUpdate (2) state?
     */
    this.phase = 0;

    /**
     * @property {boolean} Body#skipTree - If true and you collide this IsoSprite against a Group, it will disable the collision check from using an Octree.
     */
    this.skipTree = false;

    /**
     * @property {boolean} Body#_reset - Internal cache var.
     * @private
     */
    this._reset = true;

    /**
     * @property {number} Body#_sx - Internal cache var.
     * @private
     */
    this._sx = sprite.scaleX;

    /**
     * @property {number} Body#_sy - Internal cache var.
     * @private
     */
    this._sy = sprite.scaleY;

    /**
     * @property {number} Body#_dx - Internal cache var.
     * @private
     */
    this._dx = 0;

    /**
     * @property {number} Body#_dy - Internal cache var.
     * @private
     */
    this._dy = 0;

    /**
     * @property {number} Body#_dz - Internal cache var.
     * @private
     */
    this._dz = 0;

    /**
     * @property {Array<Point3>} Body#_corners - The 8 corners of the bounding cube.
     * @private
     */
   ...
   */
  updateBounds() {
    const asx = Math.abs(this.sprite.scaleX);
    const asy = Math.abs(this.sprite.scaleY);
  ...
  postUpdate() {
    ...
    if (this.deltaAbsX() >= this.deltaAbsY() && this.deltaAbsX() >= this.deltaAbsZ()) {
      if (this.deltaX() < 0) {
        this.facing = BACKWARD_X;
      } else if (this.deltaX() > 0) {
        this.facing = FORWARD_X;
      }
    } else if (this.deltaAbsY() >= this.deltaAbsX() && this.deltaAbsY() >= this.deltaAbsZ()) {
      if (this.deltaY() < 0) {
        this.facing = BACKWARD_Y;
      } else if (this.deltaY() > 0) {
        this.facing = FORWARD_Y;
      }
    } else {
      if (this.deltaZ() < 0) {
        this.facing = DOWN;
   ...
   *
   * @method Body#onFloor
   * @returns {boolean} True if in contact with either the world bounds.
   */
  onFloor() {
   ...
   *
   * @method Body#onWall
   * @returns {boolean} True if in contact with world bounds.
   */
  onWall() {
   ...
   * @returns {Array<Point3>} An array of Point3 values specifying each corner co-ordinate.
   */
  getCorners() {
  ...
  debugRender(context, color = 'rgba(0,255,0,0.4)', filled = true) {
    ...
    if (filled) {
      ...
      points = points.map(p => {
        let newPos = projector.project(p);
        newPos.x += posX;
        newPos.y += posY;
        return newPos;
      });
...
//     if (sprite.body && sprite.body.type === TYPE) {
//       this.start();
